 ...
 * - Now prompts the user until any text is given
 * - If text consists of spaces only, program exits with code 1
 * - Program now ignores multiple spaces in a row
*/
...
int main(void)
{
    ...
    // If no word is found, exit program with code 1
    if (W < 1)
    {
...
int count_words(string text)
{
    ...
    // If single character was inputted, check if its alphabetical or not
    if (len == 1)
    {
    ...
    // Iterate the string looking for valid spaces
    for (int i = 1; i < len; i++)
    {
    ...
    // Check if an extra word is added at the end of the text
